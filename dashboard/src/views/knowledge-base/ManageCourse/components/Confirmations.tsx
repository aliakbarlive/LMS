...
const Confirmations = ({ data }: { data: CategorizedArticles[] }) => {
    ...
    const articleDeleteConfirmation = useAppSelector(
        (state) =>
            state.knowledgeBaseManageCourse.data.articleDeleteConfirmation
    )
    const categoryDeleteConfirmation = useAppSelector(
        (state) =>
            state.knowledgeBaseManageCourse.data.categoryDeleteConfirmation
    )
    const categoryRenameDialog = useAppSelector(
        (state) => state.knowledgeBaseManageCourse.data.categoryRenameDialog
    )
    const categoryAddDialog = useAppSelector(
        (state) => state.knowledgeBaseManageCourse.data.categoryAddDialog
    )
    const selected = useAppSelector(
        (state) => state.knowledgeBaseManageCourse.data.selected
    )
