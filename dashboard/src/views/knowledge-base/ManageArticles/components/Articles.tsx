...
import { setMode } from '../../EditArticle/store'
...
const CategorySection = ({ data }: { data: CategorizedArticles }) => {
    ...
    const onArticleEdit = (id: string) => {
        ...
        const url=  `${window.location.origin}/dashboard/edit-article?id=${id}&categoryLabel=${data.label}&categoryValue=${data.value}&categoryId=${data._id}`
    ...
    const onArticleAdd = () => {
        ...
        navigate(
            `/edit-article?categoryLabel=${data.label}&categoryValue=${data.value}&categoryId=${data._id}`
        )
    }
    const onArticleView = (id: string) => {
        navigate(
            `/article?id=${id}`
        )
    }
    ...
    const onCategoryRename = () => {
        dispatch(setSelected({ id: data._id, categoryValue: data.value }))
